...
$("body").on("click","#save_program",function(){
	...
	var ajaxurl = $(this).attr("data-url");
	if($("#program-id").val() != ''){
		alert("Program already generated.");
	}else if( program_name != "" && program_duration != "") {
		var curr_data = { name : program_name, id : program_id, duration : program_duration };
		$.ajax({
			url : ajaxurl,
			type : "POST",
			data : curr_data,
			success : function(response){					
					if(response) {
						...
						if(response.status === "success"){
							...
							var htm = '';
							for (var i = 1; i <= program_duration; i++) {
								htm += '<option>' + i + '</option>';	
							}
							$('.wrkout-days-select').append(htm);
					        //$('.wrkout-days-select').multiselect('rebuild');
							$('.wrkout-days-select option:eq(0)').attr('selected','selected');
							$('#save_program').addClass('disabled');
		...
		alert("Please Enter Program Name & duration.");
	}
	
});

$("body").on("change",".cat_list",function(){
	var cat_id = $(this).val();
	var ct_id = $(this).attr("id").substring(19);
	if(cat_id.length > 0){
		var curr_data = { c_id : cat_id };
		var ajaxurl = "/gym/gym-ajax/getExerciseByCategory";
		$.ajax({
			url : ajaxurl,
			type : "POST",
			data : curr_data,
			success : function(response){					
					if(response) {
						...
						if(response != null){
							...
							//alert("dd "+htm);
							$('#wrkout-exercise_id-'+ct_id).html('');
							$('#wrkout-exercise_id-'+ct_id).html(htm);
						}else{

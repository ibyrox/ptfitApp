...
class ActivityController extends AppController
{
	public function activityList()
	{
		...
		if($session["role_name"]=="member")
		{
		    ...
		    $staff = $this->Activity->find("list",["keyField"=>"id","valueField"=>"name"])->where(["assigned_to"=>$session['id']]);
		    $data = $this->Activity->find()->where(["assigned_to"=>$session['id']]);
		    //$data = $this->Activity->find("all")->where(["assigned_to"=>$session['id']])->hydrate(false)->toArray();
		}else{
	...
	{		
		...
		{			
			...
			$this->request->data["assigned_to"] = $session["id"];

...
Class GymMemberController extends AppController
{
	...
	public function editMember($id) {
	    ...
	    $this->set("membership",$membership);
	    $this->set("referrer_by",$staff);
	    //$this->set("assign_group_name",$groups[$data['assign_group']]);
	    $this->set("assign_staff_name",$staff[$data['assign_staff_mem']]);
	    
        if($session["role_name"] == "administrator"){
            $groups = $this->GymMember->GymGroup->find("list",["keyField"=>"id","valueField"=>"name"]);
            $this->set("groups",$groups);
            $this->editMembersByAdmin($id);
        }else if($session["role_name"] == "staff_member"){
            $uid = intval($session["id"]);
            $groups = $this->GymMember->GymGroup->find("list",["keyField"=>"id","valueField"=>"name"])->where(["trainer_id"=>$uid])->hydrate(false)->toArray();
            $this->set("groups",$groups);
            $this->render("editClient");
            $this->editMembersByTrainer($id);
        }else if($session["role_name"] == "member"){
            $this->render("editClient");
            $this->editMembersByMember($id);
        }
	    
	}
	
	public function editMembersByMember($id) {
	...
	public function editMembersByTrainer($id)
	{
	    if($this->request->is("post")) {
	        $row = $this->GymMember->get($id);
	        $image = $this->GYMFunction->uploadImage($this->request->data['image']);
	...
	public function editMembersByAdmin($id)
	{
	    if($this->request->is("post")) {
	        $row = $this->GymMember->get($id);
	        $image = $this->GYMFunction->uploadImage($this->request->data['image']);

...
Class GymAjaxController extends AppController
{
	...
	public function getExerciseByCategory(){
	    $this->autoRender = false;
	    if($this->request->is("ajax")) {
	        $category_id = $this->request->data["c_id"];
	        $gym_exercise_tbl = TableRegistry::get("gym_exercise");
	        $exercises = $gym_exercise_tbl->find("list",["keyField"=>"id","valueField"=>"name"])->where(["category_id"=>$category_id])->hydrate(false)->toArray();

		...
		<table class="mydataTable table table-striped">
			...
			<?php
				foreach($data as $row)
				{
					...
					if($session["role_name"] == "administrator")
					{	
					    echo "<a href='{$this->request->base}/GymMember/deleteMember/{$row['id']}' title='Delete' class='btn btn-flat btn-danger' onClick=\"return confirm('Are you sure,You want to delete this record?');\"><i class='fa fa-trash-o'></i></a>";
					}
